...
// Legacy — kept for calice panel (no longer uses a global multiplier, panel removed)
let _caliceMoltiplicatore = 2.5;
let modalWine = null;
let notifTimer = null;
...
let _mobTipo = {}; // wineId → 'bottiglia' | 'mescita' (default bottiglia)
...
function inferPaese(nazione, regione, zona){
  ...
  const r=(regione||zona||"").toLowerCase().trim();
  if(!r) return "";
...
function calcRicavoMovimento(m,w){
  // Per mescita usa ricavoMescita salvato, oppure calice*calicePerBt*qty se prezzoCalice disponibile
  if(m.sottoTipo==='mescita'){
    if(m.ricavoMescita!=null) return m.ricavoMescita;
    const pc=parseFloat(w?.prezzoCalice)||0;
    const cpb=parseInt(w?.calicePerBt)||5;
    return pc?m.qty*pc*cpb:m.qty*(parseFloat(w?.prezzoCarta)||0);
  }
...
function _initSupabase(){
  try{
    const url = localStorage.getItem("cm_sb_url")||"https://aznqjmhzbehpmvxnxbzs.supabase.co";
    const key = localStorage.getItem("cm_sb_key")||"sb_publishable_FnsZcIMLdfbaABqmwx3I2A_rXfRmHhY";
...
function resetTipologie(){
  const def = ["Rosso","Bianco","Rosato","Champagne","Metodo Classico","Metodo Classico Rosato","Rifermentato","Rifermentato Rosso","Rifermentato Rosato","Col Fondo","Ancestrale","Macerato","Orange","Passito","Dolce","Liquoroso"];
  TIPOLOGIE.length = 0;
  def.forEach(t=>TIPOLOGIE.push(t));
  _saveTipologie();
...
function _saveLocalBackup(){
  try{
    ...
    localStorage.setItem("cm_fallate",JSON.stringify(fallate));
    localStorage.setItem("cm_alert_soglie",JSON.stringify(alertSoglie));
...
function scheduleSave(){
  ...
  saveTimer = setTimeout(async ()=>{
    _saveLocalBackup(); // always write local backup first
    if(!_sb){ return; }
    ...
    try{
      await Promise.all([
        ...
        _sbUpsert("cm_fallate",  { user_id:DB_USER, data:fallate }),
        _sbUpsert("cm_soglie",   { user_id:DB_USER, data:alertSoglie }),
        _sbUpsert("cm_orders",   { user_id:DB_USER, data:orders }),
      ]);
...
async function loadData(){
  ...
  try{
    const [w,m,f,s,o] = await Promise.all([
      _sbRead("cm_wines"), _sbRead("cm_movements"), _sbRead("cm_fallate"),
      _sbRead("cm_soglie"), _sbRead("cm_orders")
    ]);
...
if(localStorage.getItem("cm_logged")==="1"){
  document.getElementById("login-screen").style.display="none";
  _applySidebarState();
  _initSupabase();
  if(_isMobile()){
...
function inlineEdit(evt, field, wineId, currentVal){
  ...
  if(td.querySelector(".inline-edit-input,.inline-edit-select")) return; // già in edit

  // Campi select
  const SELECT_FIELDS = {
    tipologia: () => TIPOLOGIE,
    iva: () => IVA_OPTIONS.map(String),
  };
  if(SELECT_FIELDS[field]){
    const orig = td.innerHTML;
    const sel = document.createElement("select");
    sel.className = "form-select inline-edit-select";
    sel.style.cssText = "width:100%;min-width:120px;font-size:11px;padding:3px 6px;font-family:inherit";
    const opts = SELECT_FIELDS[field]();
    opts.forEach(o => { const op=document.createElement("option"); op.value=o; op.textContent=o; if(String(o)===String(currentVal)) op.selected=true; sel.appendChild(op); });
    // "Altro/Nuova" per tipologia
    if(field==="tipologia"){ const op=document.createElement("option"); op.value="__new__"; op.textContent="+ Nuova…"; sel.appendChild(op); }
    td.innerHTML = ""; td.appendChild(sel); sel.focus();
    const commitSel = () => {
      if(sel.value==="__new__"){ _addTipologiaInline(sel, v=>{ wines=wines.map(w=>w.id===wineId?{...w,tipologia:v}:w); scheduleSave(); renderInventarioOnly(); notify(`✏️ tipologia → ${v}`); }); return; }
      const val = sel.value;
      if(val===String(currentVal)){ td.innerHTML=orig; return; }
      wines=wines.map(w=>w.id===wineId?{...w,[field]:field==="iva"?parseInt(val):val}:w);
      scheduleSave(); renderInventarioOnly(); notify(`✏️ ${field} → ${val}`);
    };
    sel.addEventListener("change", commitSel);
    sel.addEventListener("blur", ()=>{ setTimeout(()=>{ if(document.activeElement!==sel) td.innerHTML=orig; },150); });
    sel.addEventListener("keydown", e=>{ if(e.key==="Enter"){ e.preventDefault(); commitSel(); } if(e.key==="Escape"){ td.innerHTML=orig; } });
    return;
  }

  const isNum = ["prezzoAcq","prezzoCarta","prezzoCalice","formato"].includes(field);
  const orig = td.innerHTML;
...
function getStats(){
  ...
  return {referenze:wines.length,giacenzaTot,valoreTot,valoreCarta,margineLordoTot,scoreBasse,esaurite,
...
function renderDashboard(){
  ...
  const startDate=new Date(2026,0,1); // Gen 2026
  ...
  const tipoPie=TIPOLOGIE.filter(t=>wines.some(w=>w.tipologia===t)).map(t=>({name:t,value:wines.filter(w=>w.tipologia===t).reduce((s,w)=>s+w.giacenza,0)})).filter(d=>d.value>0);
  ...
  const kpis=[
    {label:"Referenze",value:s.referenze,sub:"vini in cantina",cls:"c-amber"},
  ...
  let html=`<div class="kpi-grid g5" style="margin-bottom:20px">${kpis.map(k=>`<div class="kpi-card"><div class="kpi-label">${k.label}</div><div class="kpi-val ${k.cls}">${k.value}</div><div class="kpi-sub">${k.sub}</div></div>`).join("")}</div>`;
...
function renderInventario(){
  ...
  else list.sort((a,b)=>_tipoIdx(a.tipologia)-_tipoIdx(b.tipologia)||a.nome.localeCompare(b.nome));
  let tfG=0;
  list.forEach(w=>{tfG+=w.giacenza});

  let html=`<div class="kpi-grid g4" style="margin-bottom:14px">
    ${[{label:"Referenze",v:s.referenze,sub:"vini in lista",cls:"c-amber"},{label:"Giacenza Totale",v:s.giacenzaTot,sub:"bottiglie",cls:"c-amber3"},{label:"Scorte Basse",v:s.scoreBasse,sub:`${s.esaurite} esaurite`,cls:s.scoreBasse>0?"c-red":"c-green"},{label:"Fallate Totali",v:s.fallateTot,sub:"bottiglie perse",cls:"c-orange"}].map(k=>`<div class="kpi-card"><div class="kpi-label">${k.label}</div><div class="kpi-val ${k.cls}">${k.v}</div><div class="kpi-sub">${k.sub}</div></div>`).join("")}
  </div>
    ...
    <div class="tbl-wrap">
      <table class="inv-table">
        <thead><tr>
          ...
          <th class="r" style="border-left:1px solid var(--border)">P.Acq</th><th class="r col-ivaincl">+IVA/bt</th><th class="r col-pcarta">P.Carta</th><th class="r col-calice">P.Calice</th><th class="r col-margperc">Marg.%</th>
        ...
        list.map((w,i_)=>{
          ...
          const groupHdr_=(invSort==="tipologia"&&filterTipo==="tutti"&&w.tipologia!==prevTipo_)?
            `<tr style="background:var(--bg)"><td colspan="13" style="padding:8px 16px 5px;border-top:2px solid rgba(255,159,10,.25);border-bottom:1px solid rgba(255,159,10,.12)"><span style="font-size:11px;letter-spacing:.22em;text-transform:uppercase;color:var(--amber);font-weight:700">${h(w.tipologia)}</span>&emsp;<span style="font-size:9px;color:var(--txt4)">${list.filter(x=>x.tipologia===w.tipologia).length} etich.</span></td></tr>`
            : "";
          const rowStr_=(w=>{ 
          ...
          return `<tr class="${isEmpty?"alert-empty":isAlert?"alert-low":isRiordino?"alert-riordino":""}" data-sel-id="${w.id}" data-wine-id="${w.id}" onclick="selectWineRow('${w.id}')" style="cursor:pointer">
            ...
            <td ondblclick="inlineEdit(event,'nome','${w.id}','${h(w.nome).replace(/'/g,"\\'")}'" title="Doppio click: modifica nome"><div style="max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px">${h(w.nome)}${(parseFloat(w.formato)||0.75)>0.75?` <span style="font-size:8px;font-weight:600;padding:1px 5px;border:1px solid rgba(0,122,255,.35);color:#60a5fa;background:rgba(0,122,255,.1);border-radius:3px;white-space:nowrap">${w.formato}L</span>`:""}</div>${w.zona?`<div class="col-zona" ondblclick="event.stopPropagation();inlineEdit(event,'zona','${w.id}','${(w.zona||'').replace(/'/g,"\\'")}'" title="Doppio click: modifica zona" style="font-size:9px;color:var(--txt4);cursor:text">${h(w.zona)}</div>`:""}</td>
            <td class="col-annata" ondblclick="inlineEdit(event,'annata','${w.id}','${w.annata||""}')" title="Doppio click: modifica annata"><span style="color:var(--amber);font-family:'Montserrat',sans-serif;white-space:nowrap;font-size:11px">${w.annata||`<span style="color:var(--txt4)">N.V.</span>`}</span></td>
            <td class="col-vitigni" ondblclick="inlineEdit(event,'vitigni','${w.id}','${(w.vitigni||'').replace(/'/g,"\\'")}'" title="Doppio click: modifica vitigni" style="color:var(--txt3);font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100px;cursor:text">${h(w.vitigni||"—")}</td>
            <td ondblclick="inlineEdit(event,'tipologia','${w.id}','${w.tipologia||''}')" title="Doppio click: modifica tipologia" style="cursor:pointer">${badge(w.tipologia)}</td>
            <td class="col-regione" title="Doppio click: modifica regione/nazione" style="color:var(--txt3);font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:110px">${w.regione?`<span ondblclick="inlineEdit(event,'regione','${w.id}','${(w.regione||'').replace(/'/g,"\\'")}'" style="cursor:text" title="Doppio click: modifica regione">${h(w.regione)}</span>`:""}${w.nazione?`${w.regione?" · ":""}<span ondblclick="inlineEdit(event,'nazione','${w.id}','${(w.nazione||'').replace(/'/g,"\\'")}'" style="color:var(--amber3);font-weight:600;cursor:text" title="Doppio click: modifica nazione">${h(w.nazione)}</span>`:""}</td>
            <td class="r" ondblclick="inlineEdit(event,'prezzoAcq','${w.id}','${w.prezzoAcq||0}')" style="border-left:1px solid var(--border);white-space:nowrap;cursor:text" title="Doppio click: modifica P.Acq">${fmt(w.prezzoAcq)}</td>
            <td class="r col-ivaincl" ondblclick="inlineEdit(event,'iva','${w.id}','${w.iva||22}')" style="color:var(--txt3);white-space:nowrap;cursor:pointer" title="Doppio click: modifica IVA%">${fmtRound(calcCostoIvaBottiglia(w))}</td>
            <td class="r col-pcarta" ondblclick="inlineEdit(event,'prezzoCarta','${w.id}','${w.prezzoCarta||0}')" style="white-space:nowrap;cursor:text;${!w.prezzoCarta?'color:var(--txt4)':''}" title="Doppio click: modifica P.Carta">${w.prezzoCarta?fmt(w.prezzoCarta):'—'}</td>
            <td class="r col-calice" ondblclick="inlineEdit(event,'prezzoCalice','${w.id}','${w.prezzoCalice||0}')" style="white-space:nowrap;cursor:text;${!w.prezzoCalice?'color:var(--txt4)':''}" title="Doppio click: modifica P.Calice">${w.prezzoCalice?fmt(w.prezzoCalice):'—'}</td>
            ...
            <td class="c" style="border-left:1px solid rgba(255,159,10,.12);background:rgba(255,159,10,.04);position:relative">
              <div style="display:flex;flex-direction:column;align-items:center;gap:1px">
                <div style="color:${isEmpty?"#FF453A":isAlert?"#fb923c":isRiordino?"#fbbf24":"var(--amber)"}" class="giacenza-big">${w.giacenza}</div>
...
function _applyCaliceGlobale(){
  let count = 0;
  wines = wines.map(w => {
    if(w.prezzoCalice) return w;
    const costoIva = calcCostoIvaBottiglia(w);
    if(!costoIva) return w;
    const cpb = parseInt(w.calicePerBt)||5;
    const pc = Math.ceil((costoIva * _getMolt(w) / cpb) * 2) / 2;
    count++;
    return {...w, prezzoCalice: pc};
  });
  scheduleSave(); render();
  notify(`✅ P.Calice impostato su ${count} vini`);
}
function _applyCaliceGlobaleAll(){
  if(!confirm(`Ricalcola P.Calice per tutti i ${wines.length} vini?\nI prezzi esistenti verranno sovrascritti.`)) return;
  let count = 0;
  wines = wines.map(w => {
    const costoIva = calcCostoIvaBottiglia(w);
    if(!costoIva) return w;
  ...
  scheduleSave(); render();
  notify(`✅ P.Calice ricalcolato su ${count} vini`);
}
...
// ─── ANNULLA SCARICO ──────────────────────────────────────────────────────────
function annullaScarico(movId){
  const m = movements.find(x => x.id === movId);
  if(!m || m.tipo !== "scarico"){ notify("Movimento non trovato","err"); return; }
  if(!confirm(`Annullare lo scarico di ${m.qty}× "${m.wineName}" del ${m.data}?\nLa giacenza verrà ripristinata.`)) return;
  // Reverse-FIFO: reintegra nei lotti a partire dall'ultimo consumato
  wines = wines.map(w => {
    if(w.id !== m.wineId) return w;
    let rem = m.qty;
    const updLots = [...(w.lots||[])].reverse().map(l => {
      if(rem <= 0) return l;
      const consumed = l.qtyCaricata - l.qtyRimanente; // quanto era stato tolto da questo lotto
      const addBack = Math.min(rem, consumed);
      rem -= addBack;
      return {...l, qtyRimanente: l.qtyRimanente + addBack};
    }).reverse();
    return {...w, giacenza: w.giacenza + m.qty, lots: updLots};
  });
  movements = movements.filter(x => x.id !== movId);
  scheduleSave();
  notify(`↩ Scarico annullato — ${m.qty}× ${m.wineName} ripristinato`);
  const sy = window.scrollY;
  render();
...
function _updateScaricoCounts(){
  ...
  if(el){
    el.innerHTML = righe > 0
      ? `<span style="color:#FF6B6B">${righe} vin${righe===1?"o":"i"}</span> · <span style="color:var(--amber)">${totBt} bottigli${totBt===1?"a":"e"}</span> da scaricare`
...
let scaricoSerata = {
  open: false,
  data: (() => { const d = new Date(); d.setDate(d.getDate()-1); return d.toISOString().split("T")[0]; })(),
  note: "",
...
function registraScaricaSerata(){
  ...
  // Validate all qtys before touching state
  ...
  righe.forEach(r => {
    const {wine, qty} = r;
    // Update wine giacenza + FIFO lots
    wines = wines.map(w => {
    ...
    const sottoTipoSer = (scaricoSerata.tipos&&scaricoSerata.tipos[wine.id])||'bottiglia';
    const ricavoMescitaSer = sottoTipoSer==='mescita'?(()=>{const pc=parseFloat(wine.prezzoCalice)||0;const cpb=parseInt(wine.calicePerBt)||5;return pc?qty*pc*cpb:null;})():undefined;
    movements.unshift({
      id: uid(), wineId: wine.id, wineName: wine.nome, produttore: wine.produttore, nazione: wine.nazione||"",
      tipo: "scarico", sottoTipo: sottoTipoSer, ricavoMescita: ricavoMescitaSer, qty, data, fattura: "", fornitore: "",
...
function renderMovimenti(){
      ...
      ${winiDisponibili.length === 0
        ...
        <table id="scarico-serata-table" style="width:100%;border-collapse:collapse">
          <thead><tr>
            ...
            <th style="padding:8px 12px;font-size:9px;letter-spacing:.18em;text-transform:uppercase;color:var(--txt3);text-align:center;background:rgba(41,37,36,.4);border-bottom:1px solid var(--border2)">Vendita</th><th style="padding:8px 12px;font-size:9px;letter-spacing:.18em;text-transform:uppercase;color:var(--txt);text-align:center;background:rgba(255,69,58,.08);border-bottom:1px solid var(--border2);min-width:100px">Scarico ✏️</th>
          </tr></thead>
          <tbody>
            ${winiDisponibili.map(w => {
              ...
              const hasVal = qNum > 0;
              return `<tr style="${hasVal ? "background:rgba(255,69,58,.05)" : ""}">
                ...
                <td style="padding:6px 12px;text-align:center;border-bottom:1px solid rgba(41,37,36,.4)"><select style="font-size:9px;padding:2px 4px;background:var(--bg3);border:1px solid var(--border2);color:var(--txt2);cursor:pointer;font-family:inherit" onchange="scaricoSerata.tipos=scaricoSerata.tipos||{};scaricoSerata.tipos['${w.id}']=this.value"><option value=\"bottiglia\" ${!(scaricoSerata.tipos&&scaricoSerata.tipos[w.id]&&scaricoSerata.tipos[w.id]==='mescita')?'selected':''}>🍾 Bottiglia</option><option value=\"mescita\" ${(scaricoSerata.tipos&&scaricoSerata.tipos[w.id]==='mescita')?'selected':''}>🥂 Mescita</option></select></td>
  ...
  let html = scaricoPanelHtml + `<div class="kpi-grid g2" style="margin-bottom:20px">
    <div class="card">
      ...
      ${movForm.tipo==="scarico"?`<div class="form-row" style="margin-top:4px"><label class="form-label">Tipo vendita</label><select class="form-select" onchange="movForm.sottoTipo=this.value;render()"><option value="bottiglia" ${(movForm.sottoTipo||'bottiglia')==='bottiglia'?'selected':''}>🍾 Bottiglia intera</option><option value="mescita" ${movForm.sottoTipo==='mescita'?'selected':''}>🥂 Mescita (calice)</option></select></div>${movForm.sottoTipo==='mescita'&&selW?`<div style="margin-top:6px;padding:8px 10px;background:rgba(14,116,144,.12);border:1px solid rgba(14,116,144,.35);display:flex;flex-direction:column;gap:6px"><div style="font-size:9px;color:#32ADE6;letter-spacing:.15em;text-transform:uppercase;margin-bottom:2px">🥂 Parametri Mescita</div><div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><div style="font-size:10px;color:var(--txt3)">P.Calice sul vino: <span style="color:#32ADE6;font-family:'Montserrat',sans-serif">${selW.prezzoCalice?fmt(selW.prezzoCalice):'<span style="color:#fb923c">⚠ Non impostato</span>'}</span></div><div style="font-size:10px;color:var(--txt3)">Calici/bt: <span style="color:var(--amber)">${selW.calicePerBt||5}</span></div></div><div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center"><label style="font-size:9px;color:var(--txt4);letter-spacing:.1em;text-transform:uppercase;white-space:nowrap">Override P.Calice €</label><input type="number" class="form-input" style="width:90px;padding:4px 8px;font-size:11px" placeholder="${selW.prezzoCalice||'0.00'}" step="0.5" min="0" value="${movForm._overrideCalice||''}" oninput="movForm._overrideCalice=this.value"><label style="font-size:9px;color:var(--txt4);letter-spacing:.1em;text-transform:uppercase;white-space:nowrap">N° Calici/bt</label><input type="number" class="form-input" style="width:70px;padding:4px 8px;font-size:11px" placeholder="${selW.calicePerBt||5}" step="1" min="1" value="${movForm._overrideCalicePerBt||''}" oninput="movForm._overrideCalicePerBt=this.value"></div><div style="font-size:10px;color:var(--txt3)">Ricavo stimato: <span style="color:#30D158;font-family:'Montserrat',sans-serif">${(()=>{const pc=parseFloat(movForm._overrideCalice)||parseFloat(selW.prezzoCalice)||0;const cpb=parseInt(movForm._overrideCalicePerBt)||parseInt(selW.calicePerBt)||5;const q=parseInt(movForm.qty)||1;return pc?fmt(q*pc*cpb):'<span style="color:#fb923c">⚠ imposta P.Calice</span>'})()}</span> (${parseInt(movForm.qty)||1} bt × ${parseInt(movForm._overrideCalicePerBt)||parseInt(selW?.calicePerBt)||5} calici × ${fmt(parseFloat(movForm._overrideCalice)||parseFloat(selW?.prezzoCalice)||0)})</div></div>`:``}`:''}
    ...
    <div class="card">
      ...
      <div style="max-height:320px;overflow-y:auto;display:flex;flex-direction:column;gap:6px">
        ...
        movements.slice(0,10).map(m=>`<div class="move-log"><span style="color:${m.tipo==="carico"?"#30D158":"#FF453A"}">${m.tipo==="carico"?"⬆":"⬇"}</span><div style="flex:1;min-width:0"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${h(m.wineName)}</div><div style="color:var(--txt4);font-size:10px">${m.data}${m.fattura?" · "+m.fattura:""}</div></div><span style="font-family:'Montserrat',sans-serif;color:${m.tipo==="carico"?"#30D158":"#FF453A"};font-size:1rem">${m.tipo==="carico"?"+":"-"}${m.qty}</span></div>`).join("")}
      </div>
    </div>
  </div>
    ...
    <div class="tbl-wrap">
      <table>
        ...
        <tbody>
          ...
          (()=>{ const wMap=Object.fromEntries(wines.map(w=>[w.id,w])); return movements.map(m=>{const wObj=wMap[m.wineId]; const wAnn=wObj?.annata||""; const costoIva=wObj?calcCostoIvaBottiglia(wObj):0; return `<tr data-sel-id="${m.id}">${selMode==='movimenti'?`<td class="cb-col"><input type="checkbox" class="cb-sel" data-id="${m.id}" onchange="toggleSel('${m.id}');_updateBulkBar()"></td>`:''}<td style="color:var(--txt2)">${m.data}</td><td><span style="font-size:9px;padding:2px 8px;border:1px solid;${m.tipo==="carico"?"background:rgba(20,83,45,.3);color:#30D158;border-color:#166534":m.sottoTipo==="mescita"?"background:rgba(14,116,144,.3);color:#32ADE6;border-color:#0e7490":"background:rgba(255,69,58,.12);color:#FF6B6B;border-color:#CC3025"}">${m.sottoTipo==="mescita"?"MESCITA":m.tipo.toUpperCase()}</span></td><td style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${h(m.wineName)}</td><td style="color:var(--txt3);font-size:10px;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${h(wObj?.vitigni||"—")}</td><td style="color:var(--amber);font-family:'Montserrat',sans-serif;white-space:nowrap">${wAnn||'<span style="color:var(--txt4)">N.V.</span>'}</td><td style="color:var(--txt2);max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${h(m.produttore||"—")}</td><td style="color:var(--amber3);font-size:10px;white-space:nowrap">${h(m.nazione||wObj?.nazione||"—")}</td><td style="color:var(--txt3)">${m.fattura||"—"}</td><td style="color:var(--txt3)">${m.fornitore||"—"}</td><td class="r" style="font-family:'Montserrat',sans-serif;color:${m.tipo==="carico"?"#30D158":"#FF453A"};font-size:1rem">${m.tipo==="carico"?"+":"-"}${m.qty}</td><td class="r" style="color:var(--txt3);white-space:nowrap">${costoIva?fmt(costoIva):"—"}</td><td class="r" style="color:var(--amber);white-space:nowrap">${m.sottoTipo==="mescita"?(m.ricavoMescita!=null?'<span style="color:#32ADE6" title="Ricavo mescita totale">'+fmt(m.ricavoMescita)+'</span>':(wObj?.prezzoCalice?'<span style="color:#32ADE6" title="Stima mescita">'+fmt(m.qty*(parseFloat(wObj.prezzoCalice)||0)*(parseInt(wObj.calicePerBt)||5))+'</span>':"—")):(wObj?.prezzoCarta?fmt(parseFloat(wObj.prezzoCarta)):"—")}</td><td style="color:var(--txt4);font-size:10px">${m.note||"—"}</td><td class="c" style="white-space:nowrap"><button onclick="openMovModal('${m.id}')" style="background:none;border:1px solid var(--border2);color:var(--txt3);font-size:11px;padding:3px 8px;cursor:pointer;font-family:inherit;transition:all .15s" onmouseover="this.style.borderColor='var(--amber3)';this.style.color='var(--amber)'" onmouseout="this.style.borderColor='var(--border2)';this.style.color='var(--txt3)'">✏️</button>${m.tipo==="scarico"?` <button onclick="annullaScarico('${m.id}')" style="background:none;border:1px solid rgba(255,69,58,.3);color:#FF453A;font-size:11px;padding:3px 8px;cursor:pointer;font-family:inherit;transition:all .15s" onmouseover="this.style.background='rgba(255,69,58,.1)'" onmouseout="this.style.background='none'" title="Annulla scarico e ripristina giacenza">↩</button>`:''}</td></tr>`; }).join(""); })() }
        </tbody>
      </table>
    </div>
  </div>
  ...
  return `<div class="card" style="margin-bottom:16px">
    ...
    ${`<div style="overflow-x:auto"><table id="ssp-table" style="width:100%;border-collapse:collapse">
      <thead><tr>
        ...
        <th style="text-align:center">Vendita</th>
      ...
      <tbody>
        ${winiDisponibili.map(w=>{
          ...
          return `<tr style="${hasVal?"background:rgba(255,69,58,.05)":""}">
            ...
            <td style="text-align:center;font-family:'Montserrat',sans-serif;font-size:1.1rem;color:var(--amber3);background:rgba(255,159,10,.05)">${w.giacenza}</td>
            <td style="text-align:center"><select style="font-size:9px;padding:2px 6px;background:var(--bg3);border:1px solid var(--border2);color:var(--txt2);cursor:pointer;font-family:inherit" onchange="scaricoSerata.tipos=scaricoSerata.tipos||{};scaricoSerata.tipos['${w.id}']=this.value">
              <option value="bottiglia" ${!(scaricoSerata.tipos&&scaricoSerata.tipos[w.id]==='mescita')?'selected':''}>&#127870; Bottiglia</option>
...
function _renderReportBody(dataSelezionata){
  ...
  const byWine = {};
  scarichi.forEach(m=>{const w=wineMap[m.wineId];const k=m.wineId||m.wineName;if(!byWine[k])byWine[k]={nome:m.wineName,produttore:m.produttore||w?.produttore||"",tipologia:w?.tipologia||"",bt:0,ricavo:0,mescita:false};byWine[k].bt+=m.qty;byWine[k].ricavo+=calcRicavoMovimento(m,w);if(m.sottoTipo==="mescita")byWine[k].mescita=true;});
  ...
  <div class="kpi-grid g2" style="margin-bottom:14px">
    ...
    <div class="card">
      ...
      <table style="width:100%;border-collapse:collapse">
        ...
        <tbody>${topWines.map(w=>`<tr style="border-top:1px solid var(--border)"><td style="padding:6px 8px"><div style="font-size:12px${w.mescita?";color:#32ADE6":""}">${h(w.nome)}</div><div style="font-size:11px;color:var(--txt4)">${h(w.produttore)}</div></td><td style="padding:6px 8px">${badge(w.tipologia)}</td><td style="padding:6px 8px;text-align:center;font-family:'Montserrat',sans-serif;color:var(--amber)">${w.bt}</td><td style="padding:6px 8px;text-align:right;color:var(--amber)">${fmt(w.ricavo)}</td></tr>`).join("")}</tbody>
      </table>
    </div>
  </div>
  <div class="card" style="padding:0">
    ...
    <div class="tbl-wrap"><table>
      <thead><tr><th>Vino</th><th>Produttore</th><th>Tipologia</th><th>Annata</th><th class="r">Bt</th><th>Vendita</th><th class="r">Ricavo</th><th class="r">Costo</th><th>Note</th></tr></thead>
      <tbody>${scarichi.map(m=>{const w=wineMap[m.wineId];const ric=calcRicavoMovimento(m,w);const cos=m.qty*calcCostoIvaBottiglia(w||{});return `<tr><td style="font-size:12px">${h(m.wineName)}</td><td style="color:var(--txt3);font-size:12px">${h(m.produttore||"—")}</td><td>${badge(w?.tipologia||"")}</td><td style="color:var(--amber);font-family:'Montserrat',sans-serif">${w?.annata||"N.V."}</td><td class="r" style="color:#FF6B6B">${m.qty}</td><td><span style="font-size:10px;font-weight:600;padding:2px 8px;border:1px solid;border-radius:4px;${m.sottoTipo==="mescita"?"background:rgba(50,173,230,.12);color:#32ADE6;border-color:rgba(50,173,230,.3)":"background:rgba(255,69,58,.1);color:#FF6B6B;border-color:rgba(255,69,58,.25)"}">${m.sottoTipo==="mescita"?"MESCITA":"BOTTIGLIA"}</span></td><td class="r" style="color:var(--amber)">${ric?fmt(ric):"—"}</td><td class="r" style="color:var(--txt3)">${cos?fmt(cos):"—"}</td><td style="color:var(--txt4);font-size:11px">${m.note||"—"}</td></tr>`;}).join("")}</tbody>
...
function exportReportSerataCSV(data){
  ...
  const headers=["Vino","Produttore","Tipologia","Annata","Nazione","Bt","Vendita","Ricavo","Costo+IVA","Margine","Note"];
  const rows=scarichi.map(m=>{const w=wineMap[m.wineId];const ric=calcRicavoMovimento(m,w);const cos=m.qty*calcCostoIvaBottiglia(w||{});return [m.wineName,m.produttore||"",w?.tipologia||"",w?.annata||"",m.nazione||w?.nazione||"",m.qty,m.sottoTipo==="mescita"?"Mescita":"Bottiglia",fmtN(ric),fmtN(cos),fmtN(ric-cos),m.note||""];});
...
function _movTipologiaChange(val){
  ...
  // If a wine is already selected, update it immediately in the wines array
  if(movForm.wineId){
    wines = wines.map(w => w.id===movForm.wineId ? {...w, tipologia:val} : w);
    scheduleSave();
  }
}
...
function registraMovimento(){
  ...
  movements=[{id:uid(),wineId,wineName:wine.nome,produttore:wine.produttore,nazione:wine.nazione||"",tipo,sottoTipo:tipo==='scarico'?(movForm.sottoTipo||'bottiglia'):undefined,ricavoMescita:tipo==='scarico'&&(movForm.sottoTipo||'bottiglia')==='mescita'?(()=>{const pc=parseFloat(movForm._overrideCalice)||parseFloat(wine.prezzoCalice)||0;const cpb=parseInt(movForm._overrideCalicePerBt)||parseInt(wine.calicePerBt)||5;return pc?q*pc*cpb:null;})():undefined,qty:q,data,fattura,fornitore,note,ts:Date.now()},...movements];
  movForm={...movForm,wineId:"",_wineText:"",_newProduttore:"",_newTipologia:"Rosso",_newPrezzoCarta:"",_tipologia:"",qty:1,fattura:"",fornitore:"",note:"",prezzoAcqLotto:"",sottoTipo:"bottiglia",_overrideCalice:"",_overrideCalicePerBt:""};
  const sy = window.scrollY;
  scheduleSave(); notify(tipo==="carico"?"📦 Carico registrato":"🍾 Scarico registrato");
  render();
  requestAnimationFrame(() => window.scrollTo(0, sy));
}
  ...
  <div class="card" style="padding:0">
    ...
    <div class="tbl-wrap"><table>
      ...
      <tbody>
        ...
        fallate.map(f=>{const wF=wines.find(w=>w.id===f.wineId);return`<tr><td style="color:var(--txt2)">${f.data}</td><td>${h(f.wineName)}</td><td style="color:var(--txt2)">${h(f.produttore||"—")}</td><td style="color:var(--amber3);font-size:10px">${h(wF?.nazione||"—")}</td><td style="color:var(--txt3)">${h(f.motivo)}</td><td class="r" style="color:#fb923c;font-family:'Montserrat',sans-serif">${f.qty}</td><td style="color:var(--txt4);font-size:10px">${f.note||"—"}</td></tr>`}).join("")}
      </tbody>
    </table></div>
  </div>`;
  return html;
...
function registraFallata(){
  ...
  scheduleSave(); notify("⚠️ Fallata registrata, giacenza aggiornata"); render();
}
...
function renderAnalytics(){
  ...
  const totMescita=vendFilt.filter(m=>m.sottoTipo==='mescita').reduce((s,m)=>s+(m.wine?calcRicavoMovimento(m,m.wine):0),0);
  const totBottiglia=vendFilt.filter(m=>m.sottoTipo!=='mescita').reduce((s,m)=>s+(m.wine?m.qty*(parseFloat(m.wine.prezzoCarta)||0):0),0);
...
function _refAutofill(refId,val,field){
  _refChange(refId,field,val);
  if(!val) return;
  const r=ordineModalData.referenze.find(x=>x.id===refId);
  if(!r) return;
  // Find wine in catalog to autofill related fields
  let found=null;
  if(field==="nomeVino") found=wines.find(w=>w.nome.toLowerCase()===val.toLowerCase());
  else if(field==="produttore"){
    // If only one wine from that producer and vino not set yet
    const byProd=wines.filter(w=>w.produttore.toLowerCase()===val.toLowerCase());
    if(byProd.length===1&&!r.nomeVino) found=byProd[0];
  }
  if(found){
    const row=document.querySelector(`tr[data-ref-id="${refId}"]`);
    if(!row) return;
    if(!r.nomeVino||field==="nomeVino"){
      if(!r.produttore&&found.produttore){r.produttore=found.produttore;const inp=row.querySelectorAll("input")[0];if(inp)inp.value=found.produttore;}
      // vitigni
      if(found.vitigni){r.vitigni=found.vitigni;const inpV=row.querySelectorAll("input")[2];if(inpV)inpV.value=found.vitigni;}
      // annata
      if(found.annata){r.annata=found.annata;const inpA=row.querySelectorAll("input")[3];if(inpA)inpA.value=found.annata;}
      r.tipologia=found.tipologia;const sel=row.querySelectorAll("select")[0];if(sel){for(const o of sel.options)o.selected=o.value===found.tipologia;}
      // regione, zona, nazione
      if(found.regione){r.regione=found.regione;const inpReg=row.querySelectorAll("input")[4];if(inpReg)inpReg.value=found.regione;}
      if(found.zona){r.zona=found.zona;const inpZona=row.querySelectorAll("input")[5];if(inpZona)inpZona.value=found.zona;}
      if(found.nazione){r.nazione=found.nazione;const inpNaz=row.querySelectorAll("input")[6];if(inpNaz)inpNaz.value=found.nazione;}
...
function _updateOrdineModalTotale(){
  ...
  if(!el) return;
  // Read current values from DOM for live calculation
  const tbody=document.getElementById("omd-refs-body");
  if(!tbody){el.textContent="";return;}
  ...
  tbody.insertAdjacentHTML("beforeend",`
    <tr data-ric-id="${newR.id}" style="border-top:1px solid var(--border);background:rgba(255,159,10,.05)">
      <td style="padding:5px 6px"><input class="form-input" style="font-size:11px" list="ric-prod-dl" autocomplete="off" placeholder="Produttore"
        oninput="_ricRefChange('${newR.id}','produttore',this.value)" onchange="_ricRefAutofill('${newR.id}',this.value,'produttore')"></td>
      <td style="padding:5px 6px"><input class="form-input" style="font-size:11px" list="ric-wine-dl" autocomplete="off" placeholder="Nome vino"
        oninput="_ricRefChange('${newR.id}','nomeVino',this.value)" onchange="_ricRefAutofill('${newR.id}',this.value,'nomeVino')"></td>
...
function _ricRefAutofill(refId,val,field){
  _ricRefChange(refId,field,val);
  if(!val) return;
  const r=ricezioneModalData.righe.find(x=>x.id===refId);
  if(!r) return;
  let found=null;
  if(field==="nomeVino") found=wines.find(w=>w.nome.toLowerCase()===val.toLowerCase());
  if(found){
    r.tipologia=found.tipologia;
    if(!r.prezzoAcq) r.prezzoAcq=found.prezzoAcq||0;
    if(!r.produttore) r.produttore=found.produttore||"";
    r.iva=found.iva||22;
    const row=document.querySelector(`tr[data-ric-id="${refId}"]`);
    if(row){
      const sels=row.querySelectorAll("select");
      if(sels[0]) for(const o of sels[0].options) o.selected=o.value===found.tipologia;
      const inps=row.querySelectorAll("input");
      if(!r._priceTouched&&inps[2]) inps[2].value=found.prezzoAcq||"";
      if(!r._prodTouched&&inps[0]) inps[0].value=found.produttore||"";
    }
  }
  _aggiornaRicTotale();
}
...
function confermaRicezioneOrdine(){
  ...
  daProcessare.forEach(r=>{
    let wine=wines.find(w=>w.nome.toLowerCase()===r.nomeVino.toLowerCase());
    if(!wine){
      wine={id:uid(),nome:r.nomeVino,produttore:r.produttore||"",distributore:"",
    ...
    const prevRic={...wine};
    wine.giacenza=(parseInt(wine.giacenza)||0)+qtyArr;
    wine.prezzoAcq=pAcq;
    if(!wine.lots) wine.lots=[];
    wine.lots.push(newLot);
    const trackedRic=_trackPriceChange(prevRic, pAcq, null, 'ricezione_ordine');
    wine.priceHistory=trackedRic.priceHistory;
    ...
    // Update qtyArr on the referenza in the order
...
function confermaRicezioneGlobale(){
  ...
  selezionati.forEach(ordine=>{
    (ordine.referenze||[]).forEach(r=>{
      ...
      let wine=wines.find(w=>w.nome.toLowerCase()===r.nomeVino.toLowerCase());
      if(!wine){
        wine={id:uid(),nome:r.nomeVino,produttore:r.produttore||"",distributore:ordine.fornitore||"",
          annata:r.annata||"",vitigni:r.vitigni||"",tipologia:r.tipologia||"Bianco",regione:r.regione||"",nazione:r.nazione||"Italia",zona:r.zona||"",
          prezzoAcq:r.prezzoAcq||0,iva:r.iva||22,prezzoCarta:r.prezzoCarta||0,giacenza:0,lots:[]};
        wines.push(wine);
      }
      ...
      wine.giacenza=(parseInt(wine.giacenza)||0)+qty;
      const prevGlob={...wine};
      wine.prezzoAcq=pAcq;
      if(!wine.lots) wine.lots=[];
      wine.lots.push(newLot);
      const trackedGlob=_trackPriceChange(prevGlob, pAcq, null, 'ricezione_globale');
      wine.priceHistory=trackedGlob.priceHistory;
      movements.unshift({id:uid(),wineId:wine.id,wineName:wine.nome,produttore:wine.produttore,nazione:wine.nazione||"",
  ...
  let html=`<div class="card card-amber" style="margin-bottom:20px">
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:18px">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end">
        ...
        <button class="btn-outline btn-sm" onclick="ricalcolaGiacenze()" style="border-color:rgba(48,209,88,.4);color:#30D158" title="Ricalcola giacenze e lotti FIFO da tutti i movimenti (ripristino)">🔄 Ricalcola Giacenze</button>
  ...
  return `
    ...
    <div class="modal-section">
      ...
      <div class="form-grid g2">
        ...
        <div><label class="form-label">Prezzo Calice € <span style="color:var(--txt4);font-size:9px;text-transform:none;letter-spacing:0">— opzionale</span></label><input class="form-input" id="mf-prezzoCalice" type="number" value="${f.prezzoCalice||''}" placeholder="0.00" step="0.01"></div>
        <div><label class="form-label">Calici per bottiglia <span style="color:var(--txt4);font-size:9px;text-transform:none;letter-spacing:0">— mescita</span></label><input class="form-input" id="mf-calicePerBt" type="number" value="${f.calicePerBt||5}" placeholder="5" step="1" min="1"></div>
  ...
  if(hint){
    if(cartaSuggerita){
      ...
      if(cartaInput&&carta===0&&!cartaInput._userEdited){
        cartaInput.value=fmtN(cartaSuggerita);
...
function applyCartaSuggerita(){
  ...
  const costoIva=pAcq*(1+iva/100);
  const molt=_getMolt({prezzoAcq:pAcq,iva,nome});
  const suggerito=Math.ceil(costoIva*molt*2)/2;
  const inp=document.getElementById("mf-prezzoCarta");
  if(inp){inp.value=fmtN(suggerito);inp._userEdited=false;updateModalCalc();}
}
function saveWine(){
...
function bulkDeleteMovimenti(){
  ...
  // FIFO full recalc: reset all stocks then replay remaining movements in chronological order
...
const _bulkFields={
  wines:[
    ...
    {key:"prezzoCalice",label:"Prezzo Calice (€)",type:"number"},
    {key:"calicePerBt",label:"Calici per bottiglia",type:"number"},
...
function exportInventarioCSV(){
  const dateStr=new Date().toLocaleDateString("it-IT");
  const headers=["Distributore","Produttore","Nome Vino","Vitigni","Annata","Zona/Cru","Regione","Tipologia","P.Acq (escl.IVA)","IVA %","Costo+IVA/bt","P.Carta","P.Calice","Marg.Lordo/bt","Marg.%","Giacenza","Val.Costo","IVA su Stock","Val.Potenziale Carta","IVA Pot.Vendita"];
  const rows=wines.map(w=>{const mb=calcMargineBottiglia(w);const mp=calcMarginePerc(w);const vc=calcValore(w);return [w.distributore||"",w.produttore,w.nome,w.vitigni||"",w.annata||"",w.zona||"",w.regione||"",w.tipologia,fmtN(w.prezzoAcq),w.iva+"%",fmtN(calcCostoIvaBottiglia(w)),fmtN(w.prezzoCarta),w.prezzoCalice?fmtN(w.prezzoCalice):"—",mb!==null?fmtN(mb):"—",mp!==null?fmtN(mp,1)+"%":"—",w.giacenza,fmtN(vc),fmtN(vc*(parseInt(w.iva)||22)/100),fmtN(calcValoreCarta(w)),fmtN(calcValoreCarta(w)*(parseInt(w.iva)||22)/100)];});
...
function _renderMobList(){
  ...
  list.innerHTML = filtered.map(w => {
    ...
    const isMescita = _mobTipo[w.id] === 'mescita';
    const hasCalice = parseFloat(w.prezzoCalice) > 0;
    // Calice hint: ricavo stimato se mescita
    const caliceHint = isMescita && hasCalice
      ? `<span style="font-size:9px;color:#32ADE6;margin-top:2px;display:block">🥂 ${fmt((parseFloat(w.prezzoCalice)||0)*(parseInt(w.calicePerBt)||5)*(qty))}</span>`
      : '';
    return `<div class="mob-wine-row">
      ...
      <div class="mob-wine-right">
        <button onclick="_mobTipo['${w.id}']=(_mobTipo['${w.id}']||'bottiglia')==='bottiglia'?'mescita':'bottiglia';_renderMobList()"
          style="font-size:9px;padding:2px 6px;border:1px solid ${isMescita?'rgba(50,173,230,.5)':'rgba(255,255,255,.12)'};background:${isMescita?'rgba(50,173,230,.15)':'none'};color:${isMescita?'#32ADE6':'var(--txt4)'};border-radius:4px;cursor:pointer;font-family:inherit;white-space:nowrap"
          title="${isMescita?'Mescita — tocca per Bottiglia':'Bottiglia — tocca per Mescita'}">${isMescita?'🥂':'🍾'}</button>
...
async function registraMovimentoMobile(wineId, direction){
  ...
  const sottoTipo = (direction < 0 && _mobTipo[wineId] === 'mescita') ? 'mescita' : 'bottiglia';
  ...
  const newMov = {id:movId, wineId, wineName:wine.nome, produttore:wine.produttore,
    tipo, sottoTipo, ricavoMescita, qty, data:dateStr, fattura, fornitore:"", note:"[mobile]", ts:Date.now()};
  ...
  if(_sb){
    ...
    try{
      ...
      const mergedWines = freshW.map(w => {
        if(w.id !== wineId) return w;
        if(delta > 0){
          const pAcq = parseFloat(w.prezzoAcq)||0;
          const newLot = {id:uid(), data:dateStr, fattura, fornitore:"", prezzoAcq:pAcq, iva:w.iva, qtyCaricata:qty, qtyRimanente:qty};
  ...
  const desc = delta < 0
    ? `Scaricato ${qty}× ${wine.nome}${wine.annata?" "+wine.annata:""}${sottoTipo==='mescita'?' 🥂':''}`
...
function saveMovEdit(){
  ...
  // Ricalcola giacenza del vino se tipo o qty sono cambiati
  if(oldMov && (oldTipo !== newTipo || oldQty !== newQty)){
    wines = wines.map(w => {
      if(w.id !== oldMov.wineId) return w;
      // Annulla vecchio effetto
      let g = w.giacenza;
      if(oldTipo === "carico")  g -= oldQty;
      else                      g += oldQty;
      // Applica nuovo effetto
      if(newTipo === "carico")  g += newQty;
      else                      g -= newQty;
      return {...w, giacenza: Math.max(0, g)};
    });
  }
...
function exportBilancioCSV(){
  ...
  lines.push(row("Produttore","Nome Vino","Tipologia","Annata","P.Acq","IVA%","P.Carta","P.Calice","Giacenza","Val.Costo","IVA Stock","Val.Carta"));
  wines.forEach(w=>{const vc=calcValore(w);lines.push(row(w.produttore,w.nome,w.tipologia,w.annata||"",fmtN(w.prezzoAcq),w.iva+"%",fmtN(w.prezzoCarta),w.prezzoCalice?fmtN(w.prezzoCalice):"—",w.giacenza,fmtN(vc),fmtN(vc*(parseInt(w.iva)||22)/100),fmtN(calcValoreCarta(w))));});
...
// ─── RICALCOLA GIACENZE FIFO ─────────────────────────────────────────────────
function ricalcolaGiacenze(){
  const nMov = movements.length;
  if(!confirm(`Ricalcola giacenze e lotti FIFO per tutti i vini dai ${nMov} movimenti?\n\nLe giacenze correnti verranno azzerate e ricalcolate in ordine cronologico.\nOK = procedi · Annulla = abort`)) return;

  // Reset tutti i vini
  wines = wines.map(w => ({...w, giacenza:0, lots:[]}));

  // Ordina movimenti per data + timestamp
  ...
  let carichiFatti = 0, scarichiFatti = 0, orphans = 0;

  sorted.forEach(m => {
    const wIdx = wines.findIndex(w => w.id === m.wineId);
    if(wIdx < 0){ orphans++; return; }
    const w = wines[wIdx];
    const q = parseInt(m.qty)||0;
    if(q <= 0) return;
  ...
  // Applica anche fallate (sono fuori movements)
  fallate.forEach(f => {
  ...
  scheduleSave();
  render();
  const totGiac = wines.reduce((s,w)=>s+w.giacenza,0);
  notify(`✅ Giacenze ricalcolate — ${carichiFatti} carichi, ${scarichiFatti} scarichi · Totale giacenza: ${totGiac} bt${orphans>0?" · ⚠️ "+orphans+" movimenti orfani (wineId non trovato)":""}`, "ok");
}
